...
#import "DSYARViewController.h"

@interface DSYMainViewController ()<UINavigationControllerDelegate>
...
@implementation DSYMainViewController
{
    CGFloat hi;
    CGFloat barhi;
}
...
-(void)setMainHeadScrollView{
    self.scrollView = [[MainHeadScrollView alloc]initWithFrame:CGRectMake(0, 64, self.view.bounds.size.width, 170) ImagesCount:5];
...
-(void)setbtn{
    ...
    for (int i=0;i<8;i++) {
        ...
        if (index== 0) {
            ...
            [btnView.btn addTarget:self action:@selector(arshow) forControlEvents:UIControlEventTouchUpInside];
        }
        else if (index== 2) {
            ...
            [btnView.btn addTarget:self action:@selector(znst) forControlEvents:UIControlEventTouchUpInside];
        }
        else if (index== 7) {
...
-(void)arshow{
    ...
    DSYARViewController* arVc = [[DSYARViewController alloc]init];
...
-(void)znst{
#pragma marking kkkkk
    DSYSmartFindImage* smart = [[DSYSmartFindImage alloc]init];
...
-(void)setbar{
    ...
    UIView* content = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_scrollView.frame), ScreenHeight, 80)];
    ...
    UILabel* time = [[UILabel alloc]initWithFrame:CGRectMake(ScreenWidth-100, content.frame.size.height-15, 100, 15)];
    time.text = [self gettime];
    time.font = [UIFont systemFontOfSize:12];
    time.textColor = [UIColor grayColor];
    
    UILabel* mingyan = [[UILabel alloc]initWithFrame:CGRectMake(80, meiriView.frame.size.height, ScreenWidth-100, 60)];
