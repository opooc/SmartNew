...
- (void)viewDidLoad {
    ...
    DJRegisterView *RegisterView = [[DJRegisterView alloc]
                                initwithFrame:CGRectMake(0, 64, ScreenWidth-64, ScreenHeight) djRegisterViewTypeSMS:DJRegisterViewTypeScanfPhoneSMS plTitle:@"请输入获取到的验证码"
                                title:@"下一步"
                                    phoneholder:@"请输入要注册的手机号"
                                hq:^BOOL(NSString *phoneStr) {
                                    if ([self isMobileNumber:phoneStr]) {
                                        [BmobSMS requestSMSCodeInBackgroundWithPhoneNumber:phoneStr andTemplate:@"智慧校园" resultBlock:^(int msgId, NSError *error) {
                                            if (error) {
                                                NSLog(@"%@",error);
                                            } else {
                                                //获得smsID
