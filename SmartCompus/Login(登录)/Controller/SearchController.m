...
- (void)viewDidLoad {
   ...
   // self.navigationController.navigationBarHidden;
    ...
    DJRegisterView *SearchView = [[DJRegisterView alloc]
                                initwithFrame:CGRectMake(0, 64, ScreenWidth-64, ScreenHeight) djRegisterViewTypeSMS:DJRegisterViewTypeNoScanfSMS plTitle:@"请输入手机号"
                                title:@"下一步"
                                  phoneholder:@"请输入您的帐号,来重置密码"
                                ...
                                hq:^BOOL(NSString *phoneStr) {
                                    ...
                                    [query whereKey:@"username" equalTo:phoneStr];
                                    [query findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
                                        ...
                                        if (array == nil) {
                                            ...
                                            [BmobSMS requestSMSCodeInBackgroundWithPhoneNumber:phoneStr andTemplate:@"updata" resultBlock:^(int msgId, NSError *error) {
                                                if (error) {
                                                    NSLog(@"%@",error);
                                                } else {
                                                    //获得smsID
...
//                                        if (isSuccessful) {
                                    BOOL isPhone = [self isMobileNumber:yzmStr  ];
                                    NSLog(@"%@",yzmStr);
                                    if (isPhone) {
                                        NSLog(@"%@",@"验证成功,可执行用户请求的操作");
                                       
                                        
                                        // updata.UyzmNum = yzmStr;
                            
                                    }
                                    else
                                    {
                                    
                                        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"伙计,手机号不正确或填写错误" delegate:self cancelButtonTitle:@"重写" otherButtonTitles:nil];
                                        [alert show];
