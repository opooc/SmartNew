...
//  智慧校园
//
//  Created by doushuyao on 2018/01/20.
...
#import "OHMySQL.h"
#import "DSYDefines.h"
@interface DSYCourseVC ()

@end

@implementation DSYCourseVC
{
...
-(void)setId{
    ...
    UILabel* idLabel = [[UILabel alloc]initWithFrame:CGRectMake((SCREEN_WIDTH-300)*0.5, CGRectGetMaxY(idView.frame)*0.5, 60, 20)];
    idLabel.text = @"学号:";
    ...
    inNum = [[Info sharedInstance]getnum];
...
-(void)idOK{
    ...
    if ([num.text  isEqual: @""]) {
        ...
        [[Info sharedInstance]savenum:num.text];
...
-(void)endprove{
        if ((![[[Info sharedInstance]getnum] isEqualToString:@""]) &&(validBtn.hidden == YES)&&(duibibtn.hidden = YES) ){
...
-(void)updateMysql{
    ...
    NSString* condition = [NSString stringWithFormat:@"num_id=%@",num];
    
    OHMySQLUser *user = [[OHMySQLUser alloc] initWithUserName:@"root"
                                                     password:@"zhuimeng"
                                                   serverName:@"106.14.199.20"
                                                       dbName:@"sa"
                                                         port:3306
    ...
    OHMySQLQueryRequest *query = [OHMySQLQueryRequestFactory UPDATE:@"Attendance" set:@{ @"situation": @"1"} condition:condition];
    NSError *error;
